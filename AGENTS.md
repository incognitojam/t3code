...
- Preserve upstream intake provenance in landed commit metadata. Intake commits use a comma-separated `Upstream-PR: 1234, 5678` trailer; fork pull requests retain their escaped source references in the squash commit body. The promotion workflow's `source_prs` input must exactly match that metadata so the tracking issue can reconcile landed work without notifying upstream.
